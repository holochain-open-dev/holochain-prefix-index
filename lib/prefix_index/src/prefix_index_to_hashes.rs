...
/// Duplicates of get_children and get_children_paths from holochain TypedPath
/// but without calling ensure() on those children
fn get_children(path: TypedPath) -> ExternResult<Vec<Link>> {
...
fn get_children_paths(path: TypedPath) -> ExternResult<Vec<TypedPath>> {
...
fn path_to_string(path: TypedPath) -> String {
